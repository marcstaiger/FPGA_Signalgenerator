...
Wie lassen sich aus einem einzigen, festen 50\,MHz-Systemtakt unterschiedlich schnelle Ausgabefrequenzen erzeugen? Dafür gibt es grundsätzlich zwei Möglichkeiten, entweder man erzeugt sich selbst einen langsameren Takt, zum Beispiel aus einem Bit eines Zählers, oder man lässt alle Register am echten Systemtakt und steuert stattdessen über ein Enable-Signal, wann ein neuer Wert übernommen wird.

Ein selbstgebauter, langsamerer Takt kann auf Dauer zu Problemen führen, die sich erst auf der echten Hardware zeigen. Die Variante mit Enable-Signal umgeht das, kostet dafür aber mehr Leistung, die Taktleitung läuft im ganzen Design durchgehend mit 50\,MHz, auch in den Teilen der Schaltung, die eigentlich viel langsamer arbeiten könnten. In diesem Projekt wird die Enable-Variante verwendet, \emph{alle} Register bleiben am echten Systemtakt, bekommen aber zusätzlich ein ganz normales Signal spendiert, das nur gelegentlich für einen Takt lang auf \texttt{1} geht, ein sogenanntes \emph{Enable-Signal}. Jedes Register prüft bei jeder Taktflanke, ob das Enable-Signal gerade aktiv ist, und übernimmt einen neuen Wert nur dann. Ist es das nicht, bleibt der alte Wert einfach stehen, obwohl der Takt weiterläuft.

Im Projekt heißt dieses Signal \texttt{tick\_en} und wird vom Modul \texttt{frequenzteiler} erzeugt. Ein Zähler läuft im Systemtakt hoch und setzt \texttt{tick\_en} für genau einen Takt auf \texttt{1}, sobald er den Wert \texttt{P2} erreicht hat. Je größer \texttt{P2}, desto seltener wird \texttt{tick\_en} aktiv, desto langsamer laufen alle Generatoren, und desto tiefer wird der erzeugte Ton. \texttt{P2} ist also der grobe, gemeinsame Geschwindigkeitsregler für das ganze System.

Wichtig dabei, der Zähler im \texttt{frequenzteiler} ist einstufig, keine Kaskade mehrerer Zähler hintereinander. Deshalb lässt sich \texttt{tick\_en} auch einfach als Teilung beschreiben, \texttt{tick\_en} hat die Frequenz $\frac{50\,\text{MHz}}{\texttt{P2}}$. Das brauchst du später bei der Frequenzberechnung.
